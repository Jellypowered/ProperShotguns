...
namespace ProperShotguns
{
    public class ProperShotgunsSettings : ModSettings
    {
        public void DoWindowContents(Rect wrect)
        {
            ...
            var shotgunDamageRoundingOpts = Enum.GetValues(typeof(ShotgunDamageRoundMode))
                .Cast<ShotgunDamageRoundMode>()
                .ToList();
            for (int i = 0; i < shotgunDamageRoundingOpts.Count; i++)
            {
                ...
                if (
                    options.RadioButton(
                        $"ProperShotguns.ShotgunDamageRounding_{curOpt}".Translate(),
